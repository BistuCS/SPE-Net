...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.forward_head(x1)
        # print("yyyyy")
        return x1, x

def get_part_pool(x, pool='avg'):
    """
    参数:
    ...
    返回:
        List[torch.Tensor]: 按 [12,10,8,5] 顺序输出的池化后特征列表
    """

    x_reshaped = x.reshape(-1, 37, 37, 768)  # 先转为 (B, 37, 37, 768)
    x = x_reshaped.permute(0, 3, 1, 2) 
    result = []
    ...
    for size in target_sizes:
        ...
        # 边界保护提取
        x_region = x[:, :, 
                  max(0, start_h) : min(H, end_h),
                  max(0, start_w) : min(W, end_w)]
        
        # 精确尺寸校准
        if x_region.shape[2:] != (size, size):
            x_region = F.interpolate(x_region, size=(size, size), mode='bilinear')
        
        # 池化并存储
        result.insert(0, torch.squeeze(pooling(x_region)))
    result_sum = torch.stack(result).sum(dim=0)  # 沿列表维度（dim=0）求和
    return result_sum
...
@register_model
def dinov2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-B/14 for DINOv2
    """
